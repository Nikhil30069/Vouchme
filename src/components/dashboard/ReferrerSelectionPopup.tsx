...
export const ReferrerSelectionPopup = ({ 
  ...
  const { findEligibleReferrersForJob, createReferralRequest } = useReferralStore();
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2">
            ...
            <span>Select Referrers for {jobRole} ({jobExperience} years)</span>
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-blue-600">
                  Referrers with more than {jobExperience + 1} years of experience in {jobRole}
                </p>
              </div>
              {eligibleReferrers.length > 0 && (
                ...
                >
                  {selectedReferrers.length === eligibleReferrers.length ? 'Deselect All' : 'Select All'}
                </Button>
              )}
            </div>
          </div>
          ...
          {!isLoading && eligibleReferrers.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                We couldn't find any referrers with more than {jobExperience + 1} years of experience in {jobRole}.
              </p>
              <p className="text-sm text-gray-500">
                Try posting a different job requirement or check back later as more professionals join the platform.
              </p>
            </div>
          )}
          ...
          {!isLoading && eligibleReferrers.length > 0 && (
            <div className="space-y-3">
              {eligibleReferrers.map((referrer) => (
                <Card key={referrer.referrer_id} className="hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                    <div className="flex items-start space-x-4">
                      {/* Checkbox */}
                      <Checkbox
                        checked={selectedReferrers.includes(referrer.referrer_id)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-600 mb-2">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>Total {referrer.total_experience_years || referrer.referrer_experience} years experience</span>
                          </div>
                        </div>
                        
                        {/* Organizations */}
                        {referrer.organizations && referrer.organizations.length > 0 && (
                        ...
                        {!referrer.organizations && !referrer.current_organization && referrer.organization && (
                          <div className="flex items-center space-x-2 text-sm text-gray-600">
                            <Building2 className="w-4 h-4" />
          ...
          {!isLoading && eligibleReferrers.length > 0 && (
            <div className="flex justify-between items-center pt-4 border-t">
              <div className="text-sm text-gray-600">
                {selectedReferrers.length} of {eligibleReferrers.length} referrers selected
              </div>
                ...
                >
                  {isSending ? (
                    ...
                    <>
                      ...
                      Send {selectedReferrers.length} Request{selectedReferrers.length !== 1 ? 's' : ''}
                    </>
                  )}
                </Button>
              </div>
