...
export const JobReferralRequest = ({
  user,
  onClose,
}: JobReferralRequestProps) => {
  const {
  ...
  const handleSelectAll = () => {
    if (selectedReferrers.size === eligibleReferrers.length) {
      ...
      setSelectedReferrers(
        new Set(eligibleReferrers.map((r) => r.referrer_id))
      );
    }
  };
  ...
  const handleSendRequests = async () => {
    ...
    try {
      ...
      toast.success(
        `Sent ${selectedReferrers.size} referral request(s) successfully!`
      );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Choose a job requirement to find eligible referrers for that
            specific role
          </CardDescription>
        </CardHeader>
          ...
          {userJobs.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mb-4">
                You need to create job requirements first to send referral
                requests
              </p>
                ...
                >
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    <div>
                      ...
                      {job.currentCtc?.toLocaleString()}
                    </div>
                    <div>
                      <span className="font-medium">Expected CTC:</span> ₹
                      {job.expectedCtc?.toLocaleString()}
                    </div>
                    <div>
              ...
              {eligibleReferrers.length > 0 && (
                <div className="flex items-center space-x-2">
                  <Button onClick={handleSelectAll} variant="outline" size="sm">
                    {selectedReferrers.size === eligibleReferrers.length
                      ? "Deselect All"
                      : "Select All"}
                  </Button>
                  <Button
          ...
          <CardContent>
            {eligibleReferrers.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-4">
                  No referrers with sufficient experience found for this role.
                  Try a different job requirement or check back later.
                </p>
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-sm text-blue-700">
                    <strong>Eligibility criteria:</strong> Referrers must have
                    more than{" "}
                    {userJobs.find((j) => j.id === selectedJobId)
                      ?.yearsOfExperience + 1}{" "}
                    years of experience in{" "}
                    {userJobs.find((j) => j.id === selectedJobId)?.role} role.
                  </p>
                </div>
              </div>
            ) : (
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                          ...
                          {referrer.job_role} ({referrer.job_experience} years
                          exp)
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                ...
                <li>
                  • The system automatically finds referrers with more
                  experience in that role
                </li>
                <li>• Choose which referrers you want to send requests to</li>
                <li>
                  • Referrers will review your profile and score you on
                  technical abilities and cultural fit
                </li>
                <li>
                  • Your strength score is calculated from all received scores
                </li>
              </ul>
            </div>
          </div>
